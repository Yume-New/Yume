...
function renderTripsList(){
  ...
  ids.forEach(function(tid){
    ...
    card.className = 'voy-card' + (isActive ? ' is-active' : '');
...
var MOB_ICONS = {
  ...
  train:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" width="18" height="18"><path d="M3 17 Q5 10 12 10 L21 10 Q22 11 22 13 L22 17 Q22 19 21 19 L3 19 Z"/><circle cx="7.5" cy="21" r="1.5"/><circle cx="16.5" cy="21" r="1.5"/></svg>',
  bus:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" width="18" height="18"><rect x="3" y="6" width="18" height="13" rx="2"/><line x1="3" y1="11" x2="21" y2="11"/><circle cx="7.5" cy="20" r="1.5"/><circle cx="16.5" cy="20" r="1.5"/></svg>',
  bateau:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" width="18" height="18"><path d="M2 18l3-7h14l3 7H2z"/><path d="M12 4v7"/><path d="M8 8l4-4 4 4"/></svg>',
  covoiturage:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" width="18" height="18"><path d="M5 17H3a2 2 0 0 1-2-2v-4l3-7h14l3 7v4a2 2 0 0 1-2 2h-2"/><circle cx="7" cy="17" r="2"/><circle cx="17" cy="17" r="2"/></svg>',
  metro:'Métro', taxi:'Taxi', pass:'Pass'
};
var MOB_LABELS = {
    ...
    return '<div class="loc-card item-wrap elocations">'
      +'<div class="loc-card-header">'
        ...
        +'<div style="flex:1;min-width:0">'
          ...
          +'<div class="loc-sub">'+(l.loueur?l.loueur+' · ':'')+l.statut+'</div>'
        +'</div>'
...
function _syncTotalNuits(){
  ...
  var sumHotels = hotels.reduce(function(s, h){ return s + (parseInt(h.nuits)||0); }, 0);
...
function renderNightsSummary(){
  ...
  var vm={};
  hotels.forEach(function(h){var k=h.ville;vm[k]=(vm[k]||0)+(parseInt(h.nuits)||0);});
  ...
  el.innerHTML=hotels.map(function(h){
    ...
    if(h.rue || h.cp || h.pays || h.ville){
      ...
      adresseLine = '<div class="hotel-adresse-structured">'
        ...
        +'<button class="map-pin-link" onclick="event.stopPropagation();goToMapPin(\'hotel\','+h.id+')" title="Voir sur la carte" style="background:none;border:none;cursor:pointer;font-size:13px;padding:0;margin-left:4px">🗺</button>'
        +'</div>';
    } else if(h.adresse){
      // Fallback legacy
      adresseLine = '<div class="hotel-adresse">'+h.adresse
        +'<button class="map-pin-link" onclick="event.stopPropagation();goToMapPin(\'hotel\','+h.id+')" title="Voir sur la carte">🗺</button></div>';
    }
    return '<div class="hotel-item item-wrap ehotel">'
      ...
      +'</div>'
      +'<div class="hotel-nights">'+(h.nuits?h.nuits+' nuits':'')+'</div>'
...
function addHotel(){
  ...
  hotels.push({id:uid(), nom:n, ville:v,
    ...
    nuits:parseInt(document.getElementById('ht-nuits').value)||0,
...
function renderLieux(){
  ...
  filtered.forEach(function(l){
    ...
    // Bouton carte
    var mapBtn='<button class="map-pin-link" style="margin-left:5px;background:none;border:none;cursor:pointer;font-size:13px;padding:0" onclick="event.stopPropagation();goToMapPin(\'lieu\','+l.id+')" title="Voir sur la carte">🗺</button>';
    ...
    // Ligne localisation : "Ville, Pays" élégant
    var locLine = l.ville + (l.pays ? ', <span style="color:var(--ink-hint)">' + l.pays + '</span>' : '');
    // Ligne adresse détaillée : rue + cp si renseignés
    var adresseDetail = '';
    if(l.rue){ adresseDetail = '<div class="place-adresse">'+l.rue+(l.cp?' '+l.cp:'')+', '+l.ville+(l.pays?', '+l.pays:'')+'</div>'; }

    var catBadge=l.categorie?'<div class="lieu-cat-badge '+_lieuCatClass(l.categorie)+'">'+_lieuCatIcon(l.categorie)+' '+l.categorie+'</div>':'';
    card.innerHTML=
  ...
  // Mettre à jour les filtres de ville dynamiquement
  _updateLieuxFilters();
}
...
function _lieuCatIcon(cat){
  var i={'Temples':'\u26e9\ufe0f','Parcs':'\U0001f333','Randonn\u00e9es':'\U0001f97e',
    'Restaurants':'\U0001f37d\ufe0f','Mus\u00e9es':'\U0001f3db\ufe0f','Shopping':'\U0001f6cd\ufe0f',
    'Onsen':'\u267e\ufe0f','Ch\u00e2teaux':'\U0001f3ef','Plages':'\U0001f3d6\ufe0f',
    'Points de vue':'\U0001f52d','March\u00e9s':'\U0001fad8','Galeries':'\U0001f5bc\ufe0f'};
  return i[cat]||'\U0001f4cd';
}
...
function _updateLieuxFilters(){
  ...
  ctrl.innerHTML=
    ...
    +cats.map(function(c){
      ...
      return '<button class="filter-btn cat-btn'+(currentFilter===c?' active':'')+'" onclick="filterPlaces(\''+c.replace(/\'/g,'')+'\',this)">'+_lieuCatIcon(c)+' '+c+' ('+n+')</button>';
    }).join('');
}
...
function addLieu(){
  ...
  lieux.push({id:uid(), emoji:emoji, nom:nom, ville:ville, visited:false,
    ...
    ouverture:ouv, fermeture:fer, note:note, pdfId:pdfId,
  ...
  ['lieu-nom','lieu-emoji','lieu-ville','lieu-ville-addr','lieu-categorie','lieu-ouverture','lieu-fermeture',
   'lieu-rue','lieu-cp','lieu-pays','lieu-note','lieu-pdf',
...
function calMove(dir){
  if(calShowYear){ calYear+=dir*10; }
    ...
    // Bloquer si on sort de la période voyage
    var period = getTripPeriod();
