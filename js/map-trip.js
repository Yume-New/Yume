...
//
// PHASE C — Routage ferroviaire réel (intégration RailRouter v2)
//   - Trains : fallback Bézier pointillé immédiat → remplacé par le
//     tracé ferroviaire RÉEL (RailRouter.fetchRoute, A* sur OSM) dès
//     réception. map-trip.js est l'UNIQUE propriétaire du rendu Leaflet
//     (les tracés rail respectent désormais le filtre Itinéraire).
//   - OSRM (covoiturage/taxi) : la ligne provisoire est retirée au
//     succès (avant : superposition permanente).
//   - _routeGen : garde anti-réponse périmée — aucun dessin asynchrone
//     ne peut fuir d'un voyage vers un autre (étanchéité).
//   - _TYPE_LABELS : emojis remplacés par abréviations (charte Yume).
//
...
// Compteur de génération : incrémenté à chaque _clearRoutes(). Les
// réponses asynchrones (RailRouter, OSRM) capturées sous une génération
// antérieure sont ignorées — empêche tout dessin périmé après un
// changement de voyage (étanchéité inter-voyages).
var _routeGen = 0;
...
// Retire une entrée précise de _routes (utilisé quand le tracé réel
// remplace le fallback Bézier d'un train)
function _removeRouteEntry(entry) {
  if (!entry) return;
  var idx = _routes.indexOf(entry);
  if (idx === -1) return;
  entry.layers.forEach(function (layer) {
    if (_map) { try { _map.removeLayer(layer); } catch (e) {} }
  });
  _routes.splice(idx, 1);
}

function _drawSingleRoute(lat1, lng1, lat2, lng2, type, label) {
  if (!_map) return;
  var isVol    = (type === 'vol');
  ...
  _routes.push({ layers: layers, type: type, label: label || '' });
  return _routes[_routes.length - 1];
}

// ── §6a-bis TRAINS — fallback Bézier & tracé ferroviaire réel ─────────

// Fallback immédiat pour les trains : courbe de Bézier pointillée
// (cohérente avec le style bateau — plus jamais de ligne droite brute).
// Retourne l'entrée _routes pour pouvoir la remplacer par le tracé réel.
function _drawTrainBezier(lat1, lng1, lat2, lng2, label) {
  if (!_map) return null;
  var color = _COLORS.train;
  var opts  = { color: color, weight: 2.2, opacity: 0.70,
                dashArray: '10 7', smoothFactor: 1 };
  var pts   = _bezierArc(lat1, lng1, lat2, lng2, 60);
  var poly  = L.polyline(pts, opts);
  poly.bindTooltip(label || '', { sticky: true, className: 'lmap-tooltip' });
  if (_showItineraire()) poly.addTo(_map);
  _routes.push({ layers: [poly], type: 'train', label: label || '' });
  return _routes[_routes.length - 1];
}

// Tracé ferroviaire réel (chemin A* renvoyé par RailRouter) :
// trait CONTINU épais — visuellement distinct du fallback pointillé.
function _drawRailPolyline(pts, label) {
  if (!_map || !pts || pts.length < 2) return null;
  var opts   = { color: _COLORS.train, weight: 3.0, opacity: 0.85, smoothFactor: 1.5 };
  var layers = [];
  _splitAntimeridian(pts).forEach(function (seg) {
    if (seg.length < 2) return;
    var poly = L.polyline(seg, opts);
    poly.bindTooltip(label || '', { sticky: true, className: 'lmap-tooltip' });
    if (_showItineraire()) poly.addTo(_map);
  ...
  _routes.push({ layers: layers, type: 'train', label: label || '', solid: true });
  return _routes[_routes.length - 1];
}

// Demande le tracé réel à RailRouter ; en cas de succès, remplace
// l'entrée fallback. Garde anti-périmé via _routeGen.
function _requestRealRail(m, fallbackEntry, label) {
  if (typeof RailRouter === 'undefined' || !RailRouter.fetchRoute) return;
  var gen = _routeGen;
  RailRouter.fetchRoute(m, {
    onRoute: function (pts) {
      if (gen !== _routeGen || !_map) return; // voyage changé entre-temps
      _removeRouteEntry(fallbackEntry);
      _drawRailPolyline(pts, label);
      _renderList();
    }
    // onFallback : silencieux — le fallback Bézier reste affiché
  });
}
...
function _osrmRoute(lat1, lng1, lat2, lng2, type, label) {
  ...
  // Cache hit
  if (_osrmCache[key]) {
    _drawOsrmPolyline(_osrmCache[key], type, label);
    return;
  }

  // Ligne droite immédiate pendant le chargement (retirée au succès)
  var pending = _drawSingleRoute(lat1, lng1, lat2, lng2, type, label);
  var gen     = _routeGen;
  ...
  fetch(url)
    ...
    .then(function(data) {
      ...
      _osrmCache[key] = pts;
      if (gen !== _routeGen || !_map) return; // voyage changé entre-temps
      _removeRouteEntry(pending);
      _drawOsrmPolyline(pts, type, label);
    })
    .catch(function() {});
}

function _drawOsrmPolyline(pts, type, label) {
  ...
  poly.bindTooltip(label || '', { sticky: true, className: 'lmap-tooltip' });
  if (_showItineraire()) poly.addTo(_map);
  _routes.push({ layers: [poly], type: type, label: label || '' });
}
  ...
  mobs.forEach(function (m) {
    ...
    if (type === 'vol' && m.segment2 && (m.segment2.dep || m.segment2.arr)) {
      ...
      _geocode(m.dep, function (r1) {
        ...
        _geocode(escale, function (r2) {
          ...
          _drawSingleRoute(r1.lat, r1.lng, r2.lat, r2.lng, 'vol',
            'Vol — ' + m.dep + ' → ' + escale + ' (tronçon 1)');
          _geocode(final, function (r3) {
            if (!r3) return;
            _drawSingleRoute(r2.lat, r2.lng, r3.lat, r3.lng, 'vol',
              'Vol — ' + escale + ' → ' + final + ' (tronçon 2)');
          });
        });
      });
    } else if (type === 'train' && m.depLat && m.depLng && m.arrLat && m.arrLng) {
      // Train avec coordonnées : fallback Bézier IMMÉDIAT, puis le
      // tracé ferroviaire réel (RailRouter A*) le remplace dès qu'il
      // arrive (~1-3 s, instantané si en cache).
      var fb = _drawTrainBezier(m.depLat, m.depLng, m.arrLat, m.arrLng, label);
      _requestRealRail(m, fb, label);
    } else if (type === 'train' && !m.depLat) {
      // Train sans coordonnées : géocoder → fallback Bézier → tracé réel
      _geocode(m.dep, function (r1) {
        if (!r1) return;
        _geocode(m.arr, function (r2) {
          if (!r2) return;
          var fb2 = _drawTrainBezier(r1.lat, r1.lng, r2.lat, r2.lng, label);
          var enriched = Object.assign({}, m, {
            depLat: r1.lat, depLng: r1.lng,
            arrLat: r2.lat, arrLng: r2.lng
          });
          _requestRealRail(enriched, fb2, label);
        });
      });
      ...
      _geocode(m.dep, function (r1) {
        ...
        _geocode(m.arr, function (r2) {
          if (!r2) return;
          _osrmRoute(r1.lat, r1.lng, r2.lat, r2.lng, type, label);
        });
      });
    } else {
      ...
      _geocode(m.dep, function (r1) {
        ...
        _geocode(m.arr, function (r2) {
          if (!r2) return;
          _drawSingleRoute(r1.lat, r1.lng, r2.lat, r2.lng, type, label);
        });
      });
    }
  });
...
// Charte Yume : zéro emoji — abréviations typographiques (2 lettres)
var _TYPE_LABELS = { vol:'VO', train:'TR', bus:'BU', bateau:'BA',
                     covoiturage:'CO', metro:'ME', taxi:'TA' };
...
function _renderList() {
  ...
  if (showR && _routes.length) {
    parts.push(_routes.map(function (r) {
      var isSolid = (r.type === 'vol') || !!r.solid;
      ...
      var legend = (r.type === 'vol') ? 'Arc géodésique'
                 : r.solid            ? 'Tracé ferroviaire réel'
                 :                      'Tracé pointillé';
      return '<div class="tmap-pin-card" style="cursor:default">'
